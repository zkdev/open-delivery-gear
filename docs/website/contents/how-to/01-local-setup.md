...
In order to start with the set-up first clone the [odg-core repository](https://github.com/open-component-model/odg-core).

```{note}
This guide focuses on running the ODG core (+ database) and one
...
refer to the [Local Setup](https://github.com/open-component-model/odg-core/blob/master/local-setup/local-setup.md)
guide instead.
```
...
## Running the ODG Core Service

### Requirements

1. (Optional) Create a new [virtual env](https://docs.python.org/3/library/venv.html) in your local odg-core repository and activate it:
...
```bash
# Running with PostgreSQL
adev runserver --port 5000 <path-to-local-odg-core-repo>/src -- --delivery-db-url postgresql+psycopg://postgres:MyPassword@127.0.0.1:5432

# Running with SQLite3
adev runserver --port 5000 <path-to-local-odg-core-repo>/src -- --delivery-db-url sqlite+aiosqlite:///filename.db
```
...
To run the extension locally, the Kubernetes cluster and the ODG core service
