...
for an [artefact group](#artefact-groups) which are due in the same
...
[artefact groups](#artefact-groups)) as well as the **due date**. Also, a
...
(issue-replicator-compliance-snapshots)=
...
{doc}`artefact-enumerator extension </contents/concepts/02-artefact-enumerator>` for more
...
via the {doc}`artefact-enumerator extension </contents/concepts/02-artefact-enumerator>`.

![Issues Overview](/res/issues-overview.svg)
...
3. *Delivery-Service*
   ...
   The responsibles retrieved from the ODG API
...
are different from those the current execution yields, can be defined via the
