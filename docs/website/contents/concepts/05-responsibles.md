...
{doc}`issue-replicator extension </contents/concepts/03-issue-replicator>` as option
...
GitHub issue already has assignees which are different from those the current
execution yields.
